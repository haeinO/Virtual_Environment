...
class wecar_planner():
    def __init__(self):
        ...
        self.subEgo_lidar = rospy.Subscriber("/lidar2D", LaserScan, self.laser_callback)
        ...
        self.is_obj=True#False ## 장애물 상태 점검
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_status==True  and self.is_obj==True: ## 차량의 상태, 장애물 상태 점검
                ...
                self.vo.get_object(0,0,0,0,0)#(self.object_num,self.object_info[0],self.object_info[1],self.object_info[2],self.object_info[3])
    ...
    def laser_callback(self,msg):
        ...
        for r in msg.ranges :
            ...
            if tmp_point.x < 1.5 and tmp_point.x > -1.5:
                if tmp_point.y < 0.5 and tmp_point.y > -0.5:
                    if (angle_print >= 0 and angle_print < 50):
                        #self.cmd_speed = 0
                        self.cmd_angle = angle_print/60*0.9
                        print("left")
                        print(self.cmd_angle)
                        #print(angle_print,tmp_point.x,tmp_point.y)
                    elif (angle_print > 300 and angle_print <=360):
                        print("right")
                        #self.cmd_speed = 0
                        self.cmd_angle = 360/angle_print*0.2
                        print(self.cmd_angle)                  
                        #print(360-angle_print,tmp_point.x,tmp_point.y)
    ...
    def speed_callback(self, data):
        ...
        Ego_speed_msg.data = self.cmd_speed
    ...
    def objectInfoCB(self,data): ## Object information Subscriber
        self.object_num=0#data.num_of_npcs+data.num_of_obstacle+data.num_of_pedestrian
        object_type=[]
        object_pose_x=[]
        object_pose_y=[]
        object_velocity=[]
