...
def findLocalPath(ref_path,status_msg):
    ...
    for i in range(len(ref_path.poses)) :
        #print(ref_path.poses[i].pose.position.x)
...
class cruiseControl:
    ...
    def acc(self,ego_vel,target_vel,status_msg):
        out_vel=target_vel
        pre_out_vel = out_vel
...
def latticePlanner(ref_path,vehicle_status,current_lane,weight):
    out_path=[]
    selected_lane=-1
    ...
    if len(ref_path.poses)>look_distance :
        ...
        #lane_off_set=[0.4,0.3,0.15,0,-0.15,-0.4,-0.8] # [3.0,2.0,1.0,0,-1.0,-2.0,-3.0]
        lane_off_set = [0.4,0.2,0.15,0,-0.15,-0.2,-0.4]
        ...
        lane_weight=weight #reference path, 6,4,2,0,2,4,6
                ...
                if  obj[0]==2 or obj[0]==1 : 
                    for path_num in range(len(out_path)) :
                        ...
                        for path_pos in out_path[path_num].poses :
                            ...
                            lane_dis= sqrt(pow(obj[1]-world_ego_vehicle_position[0],2)+pow(obj[2]-world_ego_vehicle_position[1],2))
                            #print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%", dis)
                            #print("----------------------------------")
                            #print("#############################", path_pos.pose)
                            #print(path_pos.pose.position)
                            #print('################',obj[1])
                            #print('*****************************',path_pos.pose.position.x)
                            #print(dis)
                            if lane_dis < 1.5:
                                if dis < 0.3: # 1.0
